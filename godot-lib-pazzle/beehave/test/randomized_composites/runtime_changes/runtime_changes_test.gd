...
func test_add_child() -> void:
	...
	assert_dict(scene.sequence_random._weights).contains_key_value(new_action.name, 1)

	# All other children's weights should be the same.
	for node in weights_before.keys():
		assert_dict(scene.sequence_random._weights).contains_key_value(node, weights_before[node])
...
func test_remove_child() -> void:
	...
	assert_dict(scene.sequence_random._weights).not_contains_keys([removed_action.name])

	# All other children's weights should be the same.
	var other_children = weights_before.keys().filter(func(k): return k != removed_action.name)
...
func test_rename_child() -> void:
	...
	assert_dict(scene.sequence_random._weights).not_contains_keys([previous_name])

	# Weights should have the new name with the default weight.
	assert_dict(scene.sequence_random._weights).contains_key_value(renamed_action.name, 1)

	# All other children's weights should be the same.
	var other_children = weights_before.keys().filter(func(k): return k != previous_name)
	for node in other_children:
		assert_dict(scene.sequence_random._weights).contains_key_value(node, weights_before[node])
...
func test_scene_reload() -> void:
	...
	for node in children:
		assert_dict(sequence_random._weights).contains_key_value(node, weights_before[node])
