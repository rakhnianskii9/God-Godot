...
## Кросс-референс с топовыми моделями (top-models.md)

Для сопоставления оценок lite-моделей с western frontier (из [top-models.md](top-models.md)):
...
² Контекстное окно top-моделей = реальный VS Code лимит (см. top-models.md). **Шкалы разные!** Gemini 3.1 Pro API = 1M, но VS Code = 173K → 5 баллов

> **⚠ ВНИМАНИЕ:** Оценки контекстного окна НЕСОПОСТАВИМЫ между документами. lite-models.md измеряет нативный контекст API (1M Gemini = 10), top-models.md измеряет реальный VS Code контекст (173K Gemini = 5). Для честного сравнения нужно выбрать одну линейку.
