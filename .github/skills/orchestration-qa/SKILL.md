...
- [ ] Conductor → exactly 5 subagents: Planning, Implement, Code-review, Proscons, UI-Governor
- [ ] Consilium-Boss → exactly 8 subagents: 4 analysts + 4 devils
- [ ] No orphan agents (every non-user-invocable agent appears in a parent's `agents:` list)
- [ ] No circular references (agent A → B → A)
- [ ] Role split is internally coherent: orchestration, implementation, review, devil, and UI governance are not duplicated or contradictory across files

### 3. Skills (`.github/skills/*/SKILL.md`)
- [ ] Every skill directory contains a `SKILL.md` file
- [ ] SKILL.md has valid frontmatter: `name`, `description`
- [ ] Skills referenced in agent routing tables actually exist as directories
- [ ] No orphan skills (every skill is referenced by at least one agent)
...
### 4. MCP Servers (`.github/mcp/config/mcp.json`)
...
- [ ] `.github/bin/start-scorecard-loop` matches the current Conductor/scorecard doctrine and contains only generic, reusable examples
- [ ] `.github/tasks.md` is the task-manager source of truth and matches paths/config referenced by agents and MCP
...
- [ ] `agents/AGENTS.md` exists and lists all agents from `.github/agents/`
- [ ] Agent count in `agents/AGENTS.md` matches actual `.agent.md` file count
- [ ] Model names in `agents/AGENTS.md` match frontmatter `model` values
- [ ] Skill routing table in Conductor matches actual skills in `.github/skills/`
...
- [ ] Startup protocol is consistent across global instructions and orchestrators: memory → task-manager → crash framing
- [ ] Memory lifecycle rules are consistent across instructions, Conductor, Consilium, and relevant subagents
- [ ] Solve-loop / completion doctrine is explicit where intended and does not contradict global instructions
- [ ] Review/scorecard/deep-analysis/diagnostics modes are consistent between parent agents and their subagents
