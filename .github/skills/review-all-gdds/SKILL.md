...
Use `vscode_askQuestions` for write permission:
- Prompt: "May I write this review to `design/gdd/gdd-cross-review-[date].md`?"
- Options: `[A] Yes — write the report` / `[B] No — skip`

If any GDDs are flagged for revision, use a second `vscode_askQuestions`:
...
append to `/memories/session/review-all-gdds.md`:
...
If `/memories/session/review-all-gdds.md` does not exist, create it with this block as the initial content.
...
After all file writes are complete, use `vscode_askQuestions` for a closing widget.
