...
- Active workspace contract — engine/project entry point configured?
- `.github/context/VERSION.md` — engine reference docs present?
...
If the project appears fresh (no artifacts at all), use `vscode_askQuestions`:
...
| Engine reference | `.github/context/VERSION.md` | HIGH — ADR engine checks blind |
...
Read the active workspace contract. Check whether engine/project details are still effectively `[TO BE CONFIGURED]`:
...
and offer to handle it immediately using `vscode_askQuestions`. Choose the first
branch that applies:
